...
//window.onload = function() {
    ...
    $('.torrent_file').change(function (){
        // var fileInput = $(".torrent_file").val();
        ...
        reader.onload = function (e) {
            ...
            $(torrent).closest(".row").find(".magnet").html(r.digestFromArrayBuffer(torrentArrayBuffer.slice(infoBegin, infoEnd)));
            document.getElementById("fileDisplayArea").innerHTML = r.digestFromArrayBuffer(torrentArrayBuffer.slice(infoBegin, infoEnd));
            // console.log(mydata);
            console.log("Info_Hash: " + r.digestFromArrayBuffer(torrentArrayBuffer.slice(infoBegin, infoEnd)));
    
            torrent_hash = r.digestFromArrayBuffer(torrentArrayBuffer.slice(infoBegin, infoEnd));
        }
        func();
        console.log(window.torrent_hash);
        // $(this).closest(".row").find(".magnet").val(window.torrent_hash);
        reader.readAsArrayBuffer(file);

        // var test = $("#fileDisplayArea").html();
        // console.log(test);
        $(this).closest(".row").find(".magnet").html(torrent_hash);
        // $("#fileDisplayArea").html("")

    });
    $("#fileDisplayArea").keyup(function torrentval($myval){
        var oldd = $("#fileDisplayArea").html()
        $myval.closest(".row").find(".magnet").html(oldd);
    });
//}
	...
	switch(str[pos]){
		...
		case 105: //char code for 'i'
			...
			for (i = pos; i < digits + pos; i++){
				val += String.fromCharCode(str[i]);
			}
			var val = Math.round(parseFloat(val));
			pos += digits + 1;
			return val;
		default:
			var digits = Array.prototype.indexOf.call(str, 58, pos) - pos; //58 = char code for ':'
			if (digits < 0 || digits > 20)
				return null;
			var len = "";
			for (i = pos; i < digits + pos; i++){
				len += String.fromCharCode(str[i]);
			}
